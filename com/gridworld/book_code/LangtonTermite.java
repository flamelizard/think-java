...
/*
Two simple rules
- on while cell, turn right, change cell to black and move forward
- on black cell, turn left, color cell white and move forward

Grid cell cannot change colors so use flowers to mimic color change
White means no flower on the cell.
 */
public class LangtonTermite extends Bug {
    Flower flower;
...
//        System.out.println(getDirection());
        ...
        act_count++;
        if (act_count % 200 == 0) {
            System.out.println("[count] " + Integer.toString(act_count));
        }
    }
...
//        Do not run over another bug
        return (isValid(next) && !(getNeighbor() instanceof Bug));
    ...
    public void move() {
        ...
        if (dropped != null) {
//            forget actor that goes beyond grid borders
            if (dropLoc == null) {
                dropped = null;
            } else {
